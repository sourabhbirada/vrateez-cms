            ...
            <div className="space-y-3 mb-5">
              ...
              {images[0] ? (
                <div className="relative h-44 rounded-xl overflow-hidden border border-border bg-surface">
                  <img src={images[0]} alt="Main product" className="h-full w-full object-cover" />
                </div>
              ) : null}
              ...
              <div className="flex gap-3 mt-4 flex-wrap">
                {images.map((img, i) => (
                  <div key={i} className="w-20 h-20 bg-surface rounded-lg border border-border relative group overflow-hidden">
                    ...
                    <button onClick={() => setImages(images.filter((_, idx) => idx !== i))} className="absolute -top-2 -right-2 w-5 h-5 bg-danger text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <X size={12} />
                    </button>
                  </div>
                ))}
