            ...
            <div key={banner._id} className="bg-card rounded-xl border border-border overflow-hidden group hover:shadow-sm transition-shadow">
              ...
              <div className="p-6 flex items-center gap-5" style={{ backgroundColor: banner.bgColor }}>
                ...
                <div className="relative w-24 h-24 bg-white/50 rounded-xl flex items-center justify-center shrink-0 overflow-hidden">
                  {banner.video ? (
                    <video src={banner.video} className="w-full h-full object-cover" muted loop autoPlay playsInline />
                  ) : banner.image ? (
        ...
        {showForm && (
          <div className="bg-card rounded-xl border border-border p-6 h-fit animate-fadeIn">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1.5">Video URL (optional)</label>
                <input type="text" value={form.video} onChange={(e) => setForm({ ...form, video: e.target.value })} placeholder="https://... (MP4 or WebM)" className="w-full px-4 py-2.5 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                <p className="text-xs text-muted mt-1">If both image and video are provided, video will be shown.</p>
                {form.video ? (
                  <div className="relative mt-3 h-24 rounded-lg overflow-hidden border border-border">
